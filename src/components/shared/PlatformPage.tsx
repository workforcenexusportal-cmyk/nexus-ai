...
export default function PlatformPage({ eyebrow, title, highlight, description, primaryCta = "Book a strategy session", primaryHref = "/book-demo", secondaryCta = "Explore platform", secondaryHref = "/services", icon = "automation", metrics = [], sections = [], faqs = [] }: PlatformPageProps) {
  ...
  return <div className="dark min-h-screen">
    ...
    <section className="mx-auto max-w-4xl px-4 py-24 text-center sm:px-6"><h2 className="text-4xl font-bold md:text-5xl">Turn operational drag into <span className="text-gradient">strategic speed.</span></h2><p className="mx-auto mt-5 max-w-2xl text-lg text-nexus-secondary">Start with one workflow. Build the intelligence layer your organization will run on.</p><div className="mt-8"><Button href="/book-demo" size="lg" showArrow>Book a strategy session</Button></div></section>
  </div>;
}
